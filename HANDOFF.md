...
- **Next.js refuses to run a second `next dev` in the same project directory, even on a different port** — it's a per-folder lock, not a per-port one (`⨯ Another next dev server is already running`, naming the existing PID). If you need a fresh process to pick up a schema change (previous bullet) but another one is already running against this same folder, there's no way to run both side by side; you have to stop the existing one (`taskkill //F //PID <pid> //T` on Windows) and start a new one in its place, checking with whoever's using it first if it isn't yours.
