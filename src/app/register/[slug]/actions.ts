...
async function findConfirmableTrade(tradeId: string) {
...
// Confirming a trade needs no form data — the holding's public fields come
// from what the sender logged, not fresh input from the recipient (brief
// section 6.4).
export async function confirmTrade(tradeId: string) {
  const match = await findConfirmableTrade(tradeId);
  if (!match) return;
  ...
  await prisma.$transaction(async (tx) => {
    // Closed holdings are never deleted (gotcha #9) — just marked released.
    await tx.pinHolding.updateMany({
      where: { pinId: trade.pinId, releasedAt: null },
    ...
    await tx.trade.update({
      where: { id: trade.id },
      // Backfills toUserId when this was an email invite to someone who
      // just signed up — links the invite to the now-authenticated account.
      data: { status: "CONFIRMED", resolvedAt: new Date(), toUserId: user.id },
    });
  });
